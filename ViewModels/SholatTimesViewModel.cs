    ...
    public async Task InitPage()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(city) && city != "Select City")
            {
                ...
                string MyCity = citySplited[1];

                var cityCodes = await _cityCodesPersistence.GetCityByName(MyCity);
                if (cityCodes.Count() == 0)
                {
                    await Shell.Current.DisplayAlert("Error", "City Not Found, Please select a valid city", "OK");
